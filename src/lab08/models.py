...
class Student:  # Объявляем класс, благодаря dataclasses удобно объявляем параметры
    ...
    def __post_init__(self):  # Именно __post_init__, потому что параметры уже указаны
        try:
            datetime.strptime(self.birthdate, "%Y-%m-%d")  # Пытаемся привести к формату YYYY-MM-DD
    ...
    def from_dict(cls, d):  # Функция создания экземпляра класса из словаря d
        ...
        if len(d) != 4:
            raise ValueError("Введено некорректное кол-во данных для созданя экземпляра класса!")

        return cls(fio=d["fio"], birthdate=d["birthdate"], group=d["group"], gpa=d["gpa"])
